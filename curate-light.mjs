...
const CORE = ['快递', '物流', '寄递', '邮政', '快件', '包裹', '网点', '分拣', '转运', '末端', '派送', '揽收', '时效', '客货邮', '进村', '冷链', '跨境'];
const COMP = ['圆通', '顺丰', '京东', '中通', '韵达', '极兔', '申通', '德邦', '菜鸟', '丰巢'];
const OPS = ['台风', '暴雨', '防汛', '防洪', '洪涝', '地质灾害', '封路', '管制', '事故', '消防', '隐患', '安全', '整治', '监管', '调度'];
...
function relevant(title = '', srcName = '', url = '') {
  ...
  // 地市"邮政管理局"源实际抓市政府页，改回关键词判定，避免泛政务噪音漏入
  return has(t, [...CORE, ...COMP, ...OPS]);
}
